...
- Sign the EIP-3009 authorization **locally** with the burner key (use the official `x402` client, or `viem` typed-data signing — **do not hand-roll** the EIP-712/3009 crypto). The key never leaves the process.
- Build the `PaymentPayload` (scheme `exact`, network `base-sepolia`) + `PaymentRequirements`; POST **that** (no `walletKey`) to `${X402_FACILITATOR_URL}/settle`. The facilitator broadcasts **and sponsors gas** → the wallet needs only USDC (0 ETH is fine).
- Keep `assertTestnetOnly` (base-sepolia only) + `extractCost` (the returned `SettlementResponse`/`PaymentRequirements` still yields atomic USDC → cents, exact-only).
- Prove it with **one tiny real settle** (a sub-cent metered call) from the funded wallet against `x402.org`.

```js
// settle.mjs (rewritten) — SIGN locally, send the SIGNED payload, NEVER the key.
const account = privateKeyToAccount(env.X402_WALLET_PRIVATE_KEY);        // key stays in-process
const paymentPayload = await exact.evm.signPayment(account, paymentRequirements); // EIP-3009 sig
const res = await fetch(`${env.X402_FACILITATOR_URL}/settle`, {
  method: "POST", headers: { "content-type": "application/json" },
  body: JSON.stringify({ x402Version: 1, paymentPayload, paymentRequirements }),   // no walletKey
});
```
